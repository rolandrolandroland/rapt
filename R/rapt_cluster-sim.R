...
clustersim <- function(under, over, rcp_rad,
  ...
  if (rho2 >= pcp) {
    stop(
      "Background density (rho2) cannot be larger than or equal to ",
  ...
  nnd <- spatstat.geom::nndist(over.scaled)
  avg.sep <- mean(nnd)
  pb.sig <- avg.sep * pb

  pb.shifts <- rgblur(
    n = spatstat.geom::npoints(over.scaled),
    method = "r", coords = "sph",
    mean = 0, sd = pb.sig
  )
  over.coo <- spatstat.geom::coords(over.scaled)
  over.coo.pb <- over.coo + pb.shifts
  over.scaled.pb <- spatstat.geom::pp3(
    x = over.coo.pb$x,
    y = over.coo.pb$y,
    z = over.coo.pb$z,
  ...
  if (is.numeric(over.nolap)) {
    print("Impossible to find no-overlap solution.")
    return(-1)
  } # change to a tryCatch
  ...
  nnR <- spatstat.geom::crosspairs.pp3(over.final, under,
    rmax = max(cr.rand.final),
...
makecluster <- function(under, over, radius1, radius2,
  ...
  if (type == "ppc") {
    ...
    over.scaled <- scaleRCP(over,
      newRadius = over.rf, oldRadius = over.r,
  ...
  # CHOOSE RADIUS METHOD
    ...
    if (speed == "slow") {
      ...
      return(list(cluster, over.scaledf, c(
        npoints(over.scaledf),
      ...
      cluster.nnR.new <- crosspairs.pp3(over.scaledf, under, cr,
        what = "indices",
      ...
      cluster.adj <- crAdjust.new(
        cluster.ind, cluster.info, diff, over.scaledf,
        under
      )
      ...
      if (more == 0) {
        ...
        cluster.ind <- randomInsert(
          cluster.ind, more, npoints(under), s,
          cluster.ind.split
        )
      }
      ...
      over.scaled.domain <- c(
        domain(under)$xrange[2] + 2 * cr,
      ...
      if (more == 0) {
        ...
        cluster.ind <- randomInsert(
          cluster.ind, more, npoints(under.new), s,
          cluster.ind.split
        )
      } else if (more < 0) {
...
hcpcluster <- function(csep_r, R, sigma1, sigma2, win, background, filepath) {
  ...
  if (background == "poisson") {
    ...
    inside.pp31 <- read.rcp(filepath[1], filepath[2],
      scaleUp = TRUE,
      newRadius = 0.50475
    )
...
morph_rods <- function(lambda,
  ...
  if (rod.spacing == "grid") {
    rod.x <- seq(min(comp.points[, 1]) - 1 - buffer,
      max(comp.points[, 1]) + 1 + buffer,
      by = 1 / rod.den[1]
    )
    rod.y <- seq(min(comp.points[, 2]) - 1 - buffer,
      max(comp.points[, 2]) + 1 + buffer,
      by = 1 / rod.den[2]
    )
    ...
    win.select <- owin(
      c(xmu - xdist / 2 - buffer, xmu + xdist / 2 + buffer),
      c(ymu - ydist / 2 - buffer, ymu + ydist / 2 + buffer)
    )
    rod.xy <- coords(rcp.scaled[inside.owin(
      rcp.scaled$x, rcp.scaled$y,
      win.select
    )])
    ...
    hex.scaled <- ppp(hex.scaled.coo$x + offset[1],
      ...
      window = owin(
        c(0, max(hex.scaled.coo$x + offset[1])),
    ...
    rod.xy <- coords(hex.scaled[inside.owin(
      hex.scaled$x, hex.scaled$y,
      win.select
    )])
    rod.xy$x <- rod.xy$x - xmu + xdist / 2
    rod.xy$y <- rod.xy$y - ymu + ydist / 2
...
morph_gb <- function(lambda,
  ...
  rcp <- read.rcp(
    paste(rcp.path, "/FinalConfig", toString(rcp.number),
      sep = ""
    ),
    ...
    if (a > 0) {
      ...
      for (i in 1:over.n) {
        if (i == 1) {
          ...
          cluster.ind[
            (sum(nPoints[1:(i - 1)]) + 1):(sum(nPoints[1:(i - 1)]) + a)
          ] <- rep(NaN, a)
          cluster.info[
            (sum(nPoints[1:(i - 1)]) + 1):(sum(nPoints[1:(i - 1)]) + a)
          ] <- rep(NaN, a)
...
over_cut <- function(X, win, cr.rand) {
  ...
  X.shift.win <- box3(
    c(-add, win[1] + add), c(-add, win[2] + add),
    c(-add, win[3] + add)
  )
...
check_vol <- function(sf, coo, vol.target, under.domain, cr.rand) {
  ...
  partin.edge.h <- cr.rand[partin.edge] -
    t(apply(bdist[partin.edge, ], 1, function(x) {
      sort(x)[1:2]
    }))
  partin.corner.h <- cr.rand[partin.corner] -
    t(apply(bdist[partin.corner, ], 1, function(x) {
      sort(x)
    }))
